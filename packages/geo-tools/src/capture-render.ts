...
import { ToolError, type CaptureRenderInput, type CaptureRenderResult } from './types.ts';
...
export async function captureRender(
	input: CaptureRenderInput & { outputDir?: string },
): Promise<CaptureRenderResult> {
	const { url } = input;
	...
	return new Promise<CaptureRenderResult>((resolve, reject) => {
		...
		proc.on('error', (err: SubprocessError) => {
			...
			if (err.code === 'ENOENT') {
				...
				reject(
					new ToolError(
						`Render subprocess error: ${err.message}`,
						'SUBPROCESS_ERROR',
					),
				);
			}
